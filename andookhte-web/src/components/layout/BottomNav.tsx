...
export function BottomNav({ onQuickAdd }: BottomNavProps) {
  ...
  return (
    ...
    >
      {/* دکمهٔ + کاملاً بالای نوار شناور است (خودش بالا رفته، نه اینکه با
          padding کل نوار را کشیده باشیم پایین — آن روش نوار را به ۱۰۵px
          می‌رساند، خیلی بلندتر از حد معمول ۵۶-۸۰px). */}
      <button
        onClick={() => {
          haptic([12, 30, 12]);
          onQuickAdd();
        ...
        className="absolute -top-16 left-1/2 z-10 grid h-14 w-14 -translate-x-1/2 place-items-center rounded-2xl bg-gradient-to-br from-brand-400 via-brand-500 to-brand-700 text-white shadow-[0_14px_30px_-10px_rgb(51_100_255/.95)] transition-transform duration-300 active:scale-90"
      >
        <span
          aria-hidden
